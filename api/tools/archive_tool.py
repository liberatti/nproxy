...
class LogArchiverTool:
    ...
    def auto_archive(cls):
        if ClusterTool.CONFIG:
            ...
            now = datetime.now(TZ)
            if 'archive' in ClusterTool.CONFIG['config'] and ClusterTool.CONFIG['config']['archive']['enabled']:
                ...
                try:
                    ...
                    trn_s = trn_dao.get_all(dt_end=dt_end, filters=arch_filter, pagination={
                        'per_page': 1000,
                        'page': 1
                    })
                    ...
                    logger.info(f"Archived {len(trn_s['data'])}/{trn_s['metadata']['total_elements']}")
                except Exception as e:
                    logger.error(e)

            if 'purge' in ClusterTool.CONFIG['config'] and ClusterTool.CONFIG['config']['purge']['enabled']:
                purge_config = ClusterTool.CONFIG['config']['purge']
                try:
                    t_purged = trn_dao.purge_before_date(now - timedelta(days=purge_config['purge_after']))
                    if t_purged>0:
...
class ElasticTool:
    def __init__(self):
        try:
            ...
            if ('username' in arch_c and len(arch_c['username'] >0)
                    and 'password' in arch_c and len(arch_c['password'] >0)
            ):
    ...
    def create_schema(self):
        with open('config/elasticsearch-schema.json', 'r') as file:
            ...
            for idx in idx_collection:
                self.database.indices.create(index=idx.pop("index_name"), body=idx, ignore=400)
    ...
    def send_transaction(self, trn):
        ...
        req_headers = {item["name"]: item["content"] for item in _trn['http']['request'].pop('headers')}
